...
// Whale paths from the Sonar logo
...
function generateSVG(course) {
  ...
  <defs>
    <linearGradient id="bg-${course.id}" x1="0" y1="0" x2="1" y2="1">
      <stop offset="0%" stop-color="${course.bg}"/>
      <stop offset="100%" stop-color="${course.bgDark}"/>
    </linearGradient>
  </defs>

  <!-- Solid color background -->
  <rect width="${WIDTH}" height="${HEIGHT}" rx="16" fill="url(#bg-${course.id})"/>

  <!-- Giant whale mark — right-aligned, fills the card -->
  <g transform="translate(380, 20) scale(5.8)" opacity="0.18">
    <path d="${WHALE_TAIL1}" fill="${WHITE}"/>
    <path d="${WHALE_TAIL2}" fill="${WHITE}"/>
    <path d="${WHALE_TAIL3}" fill="${WHITE}"/>
    <path d="${WHALE_BODY}" fill="${WHITE}"/>
  </g>

  <!-- Label — huge, bold, left-aligned -->
  <text x="60" y="310" font-family="Poppins, Arial, sans-serif" font-weight="900" font-size="90" fill="${WHITE}" letter-spacing="-1">${course.label}</text>

  <!-- "CERTIFIED" small tag above -->
  <text x="64" y="210" font-family="Poppins, Arial, sans-serif" font-weight="600" font-size="22" fill="${WHITE}" opacity="0.6" letter-spacing="6">SONAR CERTIFIED</text>
</svg>`;
}
